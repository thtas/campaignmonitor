...
class CampaignMonitor {
  ...
  protected function __construct($api_key = FALSE, $client_id = FALSE) {    
    ...
    $account = \Drupal::config('campaignmonitor.account')->get();
    ...
    // Enable logging.
    $setting = \Drupal::config('campaignmonitor.general')->get();
   ...
   * library is not found an error message is set and FALSE is returned.
   *
   ...
   */
  protected function libraryLoad($file) {
  ...
  private function getCacheTimeout() {
    $options = \Drupal::config('campaignmonitor.general')->get();
  ...
  public function getLists() {
    if (empty($this->lists)) {
      ...
      if (($cache = \Drupal::cache()->get('campaignmonitor_lists')) && !empty($cache->data)) {
